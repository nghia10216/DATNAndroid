...
class ThongTinNganhFragment : Fragment(), OnItemRecyclerViewClickListener<ThongTinNganh>, ThongTinNganhContract.View {
   ...
   private var thongTinNganhPresenter: ThongTinNganhContract.Presenter? = null
    ...
    override fun onItemLongClickListener(item: ThongTinNganh?) {
        TODO("Not yet implemented")
    }

    override fun onSuccess(data: MutableList<ThongTinNganh>) {
        for (index in data.indices) {
            data[index].nganh?.tenNganh?.let { Suggestion(it, index) }?.let { mSuggestions?.add(it) }
