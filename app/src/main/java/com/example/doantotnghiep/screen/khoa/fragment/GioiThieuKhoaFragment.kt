...
class GioiThieuKhoaFragment : Fragment(){
    ...
    private fun initView() {
        ...
        hoptacRecyclerView.layoutManager = LinearLayoutManager(context, LinearLayoutManager.HORIZONTAL, false)
    ...
    private fun initData() {
        ...
        if (bundle != null) {
            ...
            Picasso.get().load(Constant.BASE_URL_IMAGE_LOGO_KHOA + khoa.logoKhoa).into(logoKhoaImageView)
            adapterHopTac.updateData(khoa.donvihoptac as MutableList<DonViHopTac>?)
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
                gioithieukhoaTextView.text = Html.fromHtml(khoa.gioiThieuKhoa, Html.FROM_HTML_MODE_COMPACT)
            } else {
